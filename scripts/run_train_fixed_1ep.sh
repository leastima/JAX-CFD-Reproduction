...
OUTPUT_DIR="$STORAGE_PATH/model_fixed_1ep"          # 新目录，与旧 broken model 区分
...
/jumbo/yaoqingyang/batman/miniconda3/envs/cfd-gpu/bin/python -u models/train.py \
...
/jumbo/yaoqingyang/batman/miniconda3/envs/cfd-gpu/bin/python -u scripts/plot_loss_and_eval_from_predict.py \
  --output_dir="$OUTPUT_DIR" \
  --baseline_dir="$STORAGE_PATH/content/kolmogorov_re_1000" \
  --baseline_nc="$STORAGE_PATH/$EVALDATA" \
  --predict_nc="$OUTPUT_DIR/predict.nc" \
  --model_label="LI_fixed_1ep_v2" \
  --max_time_steps=200 \
