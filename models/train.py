...
flags.DEFINE_integer('train_steps', -1, help='Fixed total training steps (overrides train_epochs if > 0)')
flags.DEFINE_float('early_stop_loss_delta', -1.0, help='Stop when |loss_change| per log_every steps < this value for early_stop_patience consecutive checks (-1 = disabled)')
flags.DEFINE_integer('early_stop_patience', 10, help='Number of consecutive checks below early_stop_loss_delta before stopping')
...
flags.DEFINE_integer('dataset_num_workers', 32, help='DataLoader num_workers; use 0 to avoid fork with JAX/netCDF.')
...
flags.DEFINE_integer('max_train_samples', -1, help='Limit number of training trajectories (-1 = use all)')
flags.DEFINE_integer('max_time_steps', -1, help='Limit trajectory length in raw frames (-1 = use all; needs >= encode_steps+1 = 65)')
...
def lr_schedule(step: jnp.ndarray, num_examples: int) -> jnp.ndarray:
    ...
    if FLAGS.train_steps > 0:
        ...
        # 最多用 10% 做 warmup，避免 warmup > total
        warmup_steps = min(warmup_steps, training_steps // 10)
    else:
    ...
    if FLAGS.use_exponential_decay:
      ...
      lr = FLAGS.train_lr_init
      decay_denom = max(training_steps - warmup_steps, 1)  # avoid 0/0 when epochs==warmup_epochs
      scaled_step = jnp.maximum(step - warmup_steps, 0) / decay_denom
...
def evaluate(
    ...
    num_workers=FLAGS.dataset_num_workers)
...
def _maybe_remove_existing_checkpoint(output_dir, step_num):
  """Remove existing checkpoint_<step> dir so Orbax save does not raise 'already exists'."""
  if output_dir is None:
    return
  path = os.path.join(output_dir, 'checkpoint_%d' % step_num)
  if os.path.isdir(path):
    try:
      shutil.rmtree(path)
      logging.info('Removed existing %s for overwrite', path)
...
def main(argv):
    ...
    num_workers=FLAGS.dataset_num_workers,
        ...
        max_time_steps=FLAGS.max_time_steps)
    ...
    try:
      ...
      # Newer dm-haiku: set_policy expects a class with __qualname__; forward
      # is a TransformedWithState instance. Skip so BatchNorm policy still applies.
      pass
    ...
    # train_state = initial_state(rng, jax.tree_util.tree_map(lambda x: x[0], batch),
    ...
    with time_activity('train'):
      for step_num in range(start_step, num_train_steps):
        ...
        # Log progress at fixed intervals or at the final step.
        is_log_step  = step_num and step_num % log_every == 0
        is_final_step = step_num == num_train_steps - 1
        if is_log_step or is_final_step:
          train_scalars = jax.tree_util.tree_map(lambda v: np.mean(v).item(),
            jax.device_get(train_scalars))
          logging.info('[Train %s/%s] %s',
            step_num, num_train_steps, train_scalars)
          # Append to train_loss.csv for plot_loss_and_eval_from_predict.py
          if FLAGS.output_dir is not None and jax.host_id() == 0:
            csv_path = os.path.join(FLAGS.output_dir, 'train_loss.csv')
            write_header = not os.path.exists(csv_path)
            with open(csv_path, 'a') as f:
              if write_header:
                f.write('step,train_loss\n')
              f.write('%d,%.6g\n' % (step_num, float(train_scalars['train_loss'])))

          # Early stopping: check loss change every log_every steps
          if FLAGS.early_stop_loss_delta > 0:
            cur_loss = float(train_scalars['train_loss'])
            if _early_stop_prev_loss is not None:
              delta = abs(cur_loss - _early_stop_prev_loss)
              if delta < FLAGS.early_stop_loss_delta:
                _early_stop_counter += 1
                logging.info('[EarlyStop] loss_delta=%.2e < %.2e (count=%d/%d)',
                             delta, FLAGS.early_stop_loss_delta, _early_stop_counter,
                             FLAGS.early_stop_patience)
                if _early_stop_counter >= FLAGS.early_stop_patience:
                  logging.info('[EarlyStop] Triggered at step %d, stopping.', step_num)
                  break
              else:
                _early_stop_counter = 0
            _early_stop_prev_loss = cur_loss

        if step_num and step_num % (log_every * 100) == 0 and jax.host_id() == 0:
          if FLAGS.output_dir is not None:
            i1, i2, i3, i4, i5 = jax_utils.unreplicate(train_state)
            save_state = SaveState(i1, i2, i3, i4)
            _maybe_remove_existing_checkpoint(FLAGS.output_dir, step_num)
    ...
    if FLAGS.do_predict and jax.host_id() == 0:
      ...
      # Bring to host numpy (C-contiguous) so netCDF can write (avoids NetCDF: HDF error)
      u_np = np.asarray(u_batch, dtype=np.float32).copy()
      v_np = np.asarray(v_batch, dtype=np.float32).copy()
      logging.info(u_np.shape)
      ...
      ds = xarray.Dataset(
        {
          'u': (('sample', 'time', 'x', 'y'), u_np),
          'v': (('sample', 'time', 'x', 'y'), v_np),
        },
        coords={
          'time': (FLAGS.delta_time * FLAGS.inner_steps *
            FLAGS.explicit_inner_steps * np.arange(u_np.shape[1])),
          'x': x,
          'y': y,
          'sample': np.arange(u_np.shape[0]),
        }
      )

      if not os.path.exists(FLAGS.output_dir):
        os.makedirs(FLAGS.output_dir)
      out_path = os.path.join(FLAGS.output_dir, FLAGS.predict_result)
      if os.path.isfile(out_path):
        os.remove(out_path)
      # 分块写入避免 netcdf4 HDF error；scipy 大数组会 int32 溢出
      n_t = u_np.shape[1]
      chunk_t = min(100, max(1, n_t // 10))
      encoding = {
        'u': {'zlib': True, 'chunksizes': (1, chunk_t, FLAGS.save_grid_size, FLAGS.save_grid_size)},
        'v': {'zlib': True, 'chunksizes': (1, chunk_t, FLAGS.save_grid_size, FLAGS.save_grid_size)},
      }
      try:
        ds.to_netcdf(out_path, engine='netcdf4', encoding=encoding)
      except Exception as e:
        logging.warning('netcdf4 write failed (%s), trying scipy', e)
        # netcdf4 可能留下损坏的 HDF5 文件，必须先删除再用 scipy 重写
        if os.path.isfile(out_path):
          os.remove(out_path)
